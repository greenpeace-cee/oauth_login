 ...
 */
function _oauth_login_civix_civicrm_config($config = NULL) {
 ...
 */
function _oauth_login_civix_civicrm_install() {
  _oauth_login_civix_civicrm_config();
  // Based on <compatibility>, this does not currently require mixin/polyfill.php.
}
 ...
 */
function _oauth_login_civix_civicrm_enable(): void {
  _oauth_login_civix_civicrm_config();
  // Based on <compatibility>, this does not currently require mixin/polyfill.php.
}
 ...
 * @return bool
 */
function _oauth_login_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _oauth_login_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _oauth_login_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _oauth_login_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _oauth_login_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _oauth_login_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _oauth_login_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _oauth_login_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
